...
class _HomeworkSubmissionsScreenState extends State<HomeworkSubmissionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: _search.isNotEmpty ? IconButton(icon: const Icon(Icons.clear_rounded), onPressed: () => setState(() { _search = ''; _searchCtrl.clear(); })) : null,
        ...
        Expanded(
          child: StreamBuilder<List<HomeworkSubmission>>(
            stream: db.allHomeworkSubmissionsStream(),
            builder: (context, snap) {
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        if (isReviewed) ...[
                          ...
                          if (s.grade != null)
                            Container(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.star_rounded, size: 16, color: _gradeColor(s.grade)), const SizedBox(width: 4), Text('${t('grade')}: ${_formatGrade(s.grade)}/10', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13, color: _gradeColor(s.grade)))]),
                            ),
                        ...
                        Row(children: [
                          FilledButton.icon(
                            ...
                            icon: Icon(isReviewed ? Icons.edit_rounded : Icons.rate_review_rounded, size: 16),
                            label: Text(t('review')),
                            style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)),
                          ),
                          const SizedBox(width: 8),
                          OutlinedButton.icon(
                            onPressed: () => _showFullCode(s),
                            icon: const Icon(Icons.code_rounded, size: 16),
                            label: Text(t('viewCode')),
                          ),
                          const Spacer(),
  ...
  void _showFullCode(HomeworkSubmission s) {
    showDialog(
      context: context,
      builder: (d) => AlertDialog(
        title: Text(s.lessonTitle, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
        content: SizedBox(width: 600, child: SingleChildScrollView(child: SelectionArea(child: Text(s.codeAnswer, style: const TextStyle(fontFamily: 'monospace', fontSize: 12, height: 1.6))))),
  ...
  Future<void> _openReviewDialog(HomeworkSubmission s) async {
    final l10n = Localizations.of<AppLocalizations>(context, AppLocalizations)!;
    final t = l10n.t;
      ...
      builder: (d) => StatefulBuilder(
        builder: (context, setState) {
          void syncGrade(String v) {
            final parsed = _parseGrade(v);
            if (parsed >= 0 && parsed <= 10) {
              setState(() => grade = parsed);
            }
          }
          return AlertDialog(
            title: Text(t('reviewHomework')),
            content: SizedBox(
              width: 420,
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('0', style: TextStyle(fontSize: 11, color: AppColors.grayLight)), Text('10', style: TextStyle(fontSize: 11, color: AppColors.grayLight))]),
                          const SizedBox(height: 8),
                          TextField(
                            controller: gradeCtrl,
                            keyboardType: const TextInputType.numberWithOptions(decimal: true),
                            decoration: InputDecoration(labelText: '${t('grade')} (0-10, استخدم , أو .)', hintText: '7,5', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.edit_rounded)),
                            onChanged: syncGrade,
                          ),
            ...
            actions: [
              ...
              FilledButton(onPressed: () => Navigator.pop(d, true), child: Text(t('save'))),
            ],
          );
        },
      ),
    ...
    final parsedGrade = _parseGrade(gradeCtrl.text);
    final finalGrade = parsedGrade.clamp(0, 10).toDouble();
    try {
      final db = context.read<DatabaseService>();
      await db.reviewHomework(s.id, grade: finalGrade, feedback: feedbackCtrl.text.trim(), reviewedBy: 'admin');
  ...
  void _confirmDelete(HomeworkSubmission s) {
    final t = Localizations.of<AppLocalizations>(context, AppLocalizations)!.t;
    showDialog(
      context: context,
      builder: (d) => AlertDialog(
        title: Text(t('deleteConfirm')),
        content: Text('${t('deleteWarning')}\n\n${s.userEmail} — ${s.lessonTitle}'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(d), child: Text(t('cancel'))),
          FilledButton(style: FilledButton.styleFrom(backgroundColor: AppColors.danger), onPressed: () async { await context.read<DatabaseService>().deleteHomeworkSubmission(s.id); if (d.mounted) Navigator.pop(d); }, child: Text(t('delete'))),
        ],
      ),
    );
  }
